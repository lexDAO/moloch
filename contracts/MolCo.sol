...
interface IList {
    function listings() external returns (uint256[] memory);
    function TokenCount() external returns (uint256);
    function TokenList() external returns (uint256[] memory);
}
...
contract Moloch is ReentrancyGuard {
    ...
    ***************/
    ...
    string public stamp; // public message from Moloch summoning - could be IPFS hash, plaintext, or JSON

    address public depositToken; // deposit token contract reference; default = wETH
    IList private tokenList = IList(0x38fA6A7e46072651e084204F7892f4414c9Eb579); // reference for deployment - rinkebyprivate tokenList = IList(0x38fA6A7e46072651e084204F7892f4414c9Eb579); // reference for deployment - rinkeby
    ...
    constructor(
        ...
        require(tokenList.TokenCount > 0, "need at least one approved token");
        require(tokenList.TokenCount <= MAX_TOKEN_WHITELIST_COUNT, "too many tokens");
        require(_proposalDeposit >= _processingReward, "_proposalDeposit cannot be smaller than _processingReward");
        
        depositToken = tokenList.listings[1];
        // NOTE: move event up here, avoid stack too deep if too many approved tokens
        emit SummonComplete(_summoner, tokenList.TokenList, now, _periodDuration, _votingPeriodLength, _gracePeriodLength, _proposalDeposit, _dilutionBound, _processingReward);

        for (uint256 i = 0; i < tokenList.TokenCount; i++) {
            require(tokenList.tokens[i] != address(0), "_approvedToken cannot be 0");
            require(!tokenWhitelist[tokenList.tokens[i]], "duplicate approved token");
            tokenWhitelist[tokenList.tokens[i]] = true;
            approvedTokens.push(tokenList.tokens[i]);
        }
